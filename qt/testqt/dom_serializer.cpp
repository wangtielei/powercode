...
//#include "dom_operations_private.h"
...
std::wstring DomSerializer::GenerateMetaCharsetDeclaration(
  ...
  return std::wstring(kDefaultMetaContent) + charset.c_str();
}

// Static.
std::string DomSerializer::GenerateMarkOfTheWebDeclaration(
    const KURL& url) {
  return std::string(kDefaultMarkOfTheWeb) + url.string().latin1().data();
}
...
std::wstring DomSerializer::GenerateBaseTagDeclaration(
  ...
  std::wstring target_declaration = base_target.empty() ? L"" :
      std::wstring(kBaseTargetDeclaration) + base_target.c_str();
  return std::wstring(kBaseTagDeclaration) + target_declaration.c_str();
}
  ...
  *need_skip = false;
  if (param->is_html_document) {
    ...
    if (element->hasTagName(WebCore::HTMLNames::metaTag)) {
      ...
      //    param->current_frame_kurl));
    } else if (element->hasTagName(WebCore::HTMLNames::baseTag)) {
      // Comment the BASE tag when serializing dom.
      //result += /* TODO StdWStringToString*/ (kStartCommentNotation);
    }
  } else {
      /*
...
#else
    result = String("<meta http-equiv=\"Content-Type\" content=\"text/html;charset=\"") + String(param->text_encoding.name())+"\">";
#endif
...
WebCore::String DomSerializer::PostActionAfterSerializeEndTag(
  ...
  if (element->hasTagName(WebCore::HTMLNames::baseTag)) {
    ...
    result += "_zsw_2";//GenerateBaseTagDeclaration(std::wstring(param->doc->baseTarget()));
  }

  return result;
}

String DomSerializer::GetMyDataBuffer() const {
    return myDataBuffer;
}

std::string DomSerializer::GetMyStdStringDataBuffer() const {
    return stdstringDataBuffer;
}
...
void DomSerializer::SaveHtmlContentToBuffer(const WebCore::String& result,
  ...
  if (encoding_result.length() + data_buffer_.size() >
    ...
    //delegate_->DidSerializeDataForFrame(param->current_frame_kurl,
...
*/
  ...
  myDataBuffer += String(encoding_result.data());
  //__android_log_print(5, "zsw", "will append str = %s.", encoding_result.data());

  const char* chars = encoding_result.data();
  std::string tmp = std::string(chars ? chars : "", encoding_result.length());
  stdstringDataBuffer += tmp; 
  //__android_log_print(5, "zsw", "will append std::string str = %s.", encoding_result.data());
}
...
void DomSerializer::OpenTagToString(const WebCore::Element* element,
  ...
  if (attrMap) {
    ...
    for (unsigned i = 0; i < numAttrs; i++) {
      ...
      String zsw_val = attribute->value();
      //__android_log_print(5, "zsw", "attribute name=(%s), attribute value=(%s).", attribute->name().toString().latin1().data(), zsw_val.latin1().data());
      ...
      if (!attribute->value().isEmpty()) {
        ...
        //__android_log_print(5, "zsw", "%s:%d, ***  need_replace=%d", __PRETTY_FUNCTION__, __LINE__, need_replace_link);
        if (need_replace_link) {
          ...
          if (attr_value.startsWith("javascript:", false)) {
            result += attr_value;
          } else if (element->hasTagName(WebCore::HTMLNames::frameTag)) {
              //todo
              AtomicString frameUrl = element->getAttribute(HTMLNames::srcAttr);
              WebCore::String str_value = param->doc->completeURL(frameUrl);



              // TODO: debug: figure out the folder name by main frame url.
              //       if the main frame url is not ended with ".html", such as:
              // www.baidu.com, then we should set a proper name for this. 
              KURL u(WebCore::ParsedURLString, str_value);
              String onlyFilename = str_value.substring(u.pathAfterLastSlash(), u.pathEnd());
              result += (local_directory_name_ + onlyFilename);
              __android_log_print(5, "zsw", "sub frame url =(%s), local_dir_name_=(%s)", result.latin1().data(), local_directory_name_.latin1().data() );
          } else {
            WebCore::String str_value = param->doc->completeURL(attr_value);
            // Check whether we local files for those link.
            LinkLocalPathMap::const_iterator it = local_links_.find(str_value);
            LinkLocalPathMap::const_iterator end = local_links_.end();
            //__android_log_print(5, "zsw", "%s:%d, ***  search str=(%s)", __PRETTY_FUNCTION__, __LINE__, str_value.latin1().data());
            if (it != end) {
              // Replace the link when we have local files.
              String path("");
              __android_log_print(5, "zsw", "path=%s, it->second=%s", path.latin1().data(), it->second.latin1().data());
              //__android_log_print(5, "zsw", "%s:%d, path=(%s), directory_name=(%s)", __PRETTY_FUNCTION__, __LINE__, path.latin1().data(), param->directory_name.latin1().data());
              ///if (!param->directory_name.isEmpty())
               /// path += "/" + param->directory_name;
              path += it->second;
              result += path;
            } else {
              // If not found local path, replace it with absolute link.
              result += str_value;
              __android_log_print(5, "zsw", "result=%s", result.latin1().data());
            }
          }
        } else {
#if 1
  ...
  //__android_log_print(5, "zsw", "%s:%d, result=%s.", __PRETTY_FUNCTION__, __LINE__, result.latin1().data());
...
void DomSerializer::BuildContentForNode(const WebCore::Node* node,
                                        SerializeDomParam* param) {
  //__android_log_print(5, "zsw", "%s:%d", __PRETTY_FUNCTION__, __LINE__);
...
DomSerializer::DomSerializer(Frame* frame,
                             bool recursive_serialization,
                             DomSerializerDelegate* delegate,
                             const std::vector<KURL>& links,
                             const std::vector<String>& local_paths,
                             const String& local_directory_name)
    : delegate_(delegate),
      recursive_serialization_(recursive_serialization),
      frames_collected_(false),
      local_directory_name_(local_directory_name) {
  // Must specify available frame.
  //__android_log_print(5, "zsw", "DomSerializer ctor, local_directory_name=(%s)", local_directory_name.latin1().data());

  /*     TODO
  DCHECK(frame);
  specified_frame_ = static_cast<FrameImpl*>(frame);
  // Make sure we have not-NULL delegate.
  DCHECK(delegate);
  // Build local resources map.
  DCHECK(links.size() == local_paths.size());
  */
  std::vector<KURL>::const_iterator link_it = links.begin();
  std::vector<String>::const_iterator path_it = local_paths.begin();
  for (; link_it != links.end(); ++link_it, ++path_it) {
    /*bool never_present =*/ local_links_.add(link_it->string(), *path_it);
...
void DomSerializer::CollectTargetFrames() {
  ...
  frames_.push_back(specified_frame_);
...
#if 0 //TODO: zsw: enable this to support multi-frames
...
#else
      Frame *web_frame = element->document()->frame();
      if (web_frame) {
          frames_.push_back(web_frame);
      }
#endif
...
bool DomSerializer::SerializeDom(Frame *frame) {
  //__android_log_print(5, "zsw", "DomSerializer::SerializeDom() enter");
...
//  KURL main_page_kurl(
//      specified_frame_->loader()->url());
    KURL main_page_kurl(frame->loader()->url());
//
...
//    Frame* current_frame = frames_[i];
    ...
    const WebCore::KURL& current_frame_kurl =
        current_frame->loader()->url();

    // Check whether we have done this document.
//    if (local_links_.find(current_frame_kurl.string()) != local_links_.end()) {
      // A new document, we will serialize it.
      did_serialization = true;
      // Get target encoding for current document.
      WebCore::String encoding = current_frame->loader()->encoding();
      ...
      SerializeDomParam param(
          ...
          current_doc,
          current_frame_kurl == main_page_kurl ?
                                local_directory_name_ :
                                String());

      //__android_log_print(5, "zsw", "DomSerializer::SerializeDom() **** current_frame_kurl=(%s)", current_frame_kurl.string().latin1().data());
      //__android_log_print(5, "zsw", "DomSerializer::SerializeDom() **** main_page_kurl=(%s)", main_page_kurl.string().latin1().data());
      ...
      __android_log_print(5, "zsw", "myDataBuffer=<<<(%s)>>>", myDataBuffer.latin1().data());
      __android_log_print(5, "zsw", "stdstringDataBuffer=<<<(%s)>>>", stdstringDataBuffer.c_str());


      // Sink the remainder data and finish serializing current frame.
      //delegate_->DidSerializeDataForFrame(current_frame_kurl, data_buffer_,
...
//  }
  ...
  //DCHECK(data_buffer_.empty());
  //delegate_->DidSerializeDataForFrame(KURL(), data_buffer_,
