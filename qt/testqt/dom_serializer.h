...
class DomSerializer {
 public:
  ...
  bool SerializeDom(Frame *frame);
  String GetMyDataBuffer() const;
  std::string GetMyStdStringDataBuffer() const;
  ...
  // contain all saved auxiliary files included all sub frames and resources.
  DomSerializer(WebCore::Frame* frame,
  ...
  static std::string GenerateMarkOfTheWebDeclaration(const KURL& url);
 ...
 private:
  ...
  String myDataBuffer;
  std::string stdstringDataBuffer;
  ...
  const String& local_directory_name_;
