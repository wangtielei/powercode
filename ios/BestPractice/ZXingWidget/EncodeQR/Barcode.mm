...
-(void)setupQRCode:(NSString *)code
{
    ...
    NSString *dir = [NSString stringWithFormat:@"%@/tmpqrcode", appSupport];
    ...
    NSString *qRFile = [dir stringByAppendingPathComponent:@"tmpqr.png"];
...
-(NSString *)getDigit:(NSString *)code withType:(BarcodeType) type
{
    ...
    if([code length] != (NSUInteger)length)
    {
        return @"";
    }
...
{    
    ...
    for (unsigned int i =0; i<3 && i < [code length]; i++) 
    {
        check = [code characterAtIndex:i];
