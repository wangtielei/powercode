...
class mysql {
    ...
    function queryData($table, $column, $where)
    {
        $sqlStr = null;
        if (isset($where))
            $sqlStr = sprintf('SELECT %s FROM %s WHERE %s', $column, $table, $where);
        else
            $sqlStr = sprintf('SELECT %s FROM %s ', $column, $table);

        writeLog("sqlStr = ".$sqlStr);
        return mysql_query($sqlStr, $this->db);
    }
    ...
    function updateRow($table, $values, $where) 
    {
        ...
        if (isset($where))
        {
            $sqlString = sprintf('UPDATE %s SET %s WHERE %s', $table, $values, $where);
        }
        else
        {
            $sqlString = sprintf('UPDATE %s SET %s', $table, $values);
        }
        writeLog("Update sqlString: ".$sqlString);
        return mysql_query($sqlString, $this->db);
    }
    ...
    {
        $sqlString = sprintf('INSERT INTO %s (`%s`) VALUES ("%s")', $table, $names, $values);
        writeLog("Insert sqlSTring: ".$sqlString);
        return mysql_query($sqlString, $this->db);
    }
    ...
    function deleteRow($table, $where) 
    {
        ...
        if (isset($where))
        {
            $sqlStr = sprintf('DELETE FROM %s WHERE %s', $table, $where);
        }
        else
        {
            $sqlStr = sprintf('DELETE FROM %s ', $table);
        }
        writeLog("sqlStr = ".$sqlStr);
        return mysql_query($sqlStr);
    }
    ...
    function escape($string) 
    {
        ...
        $result = mysql_real_escape_string($string, $this->db);

        writeLog("before escape: ".$string);
        writeLog("after escap: ".$result);
    ...
    {
        writeLog("sqlCmd = ".$sqlCmd);
        return mysql_query($sqlCmd);
    }
}
?>
